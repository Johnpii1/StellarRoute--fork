...
fn test_policy_comparison() {
    ...
    for (name, policy) in policies {
        ...
        let result = optimizer.find_optimal_routes("XLM", "BTC", &edges, 100_000_000).unwrap();
...
fn test_deterministic_behavior() {
    ...
    let result1 = optimizer.find_optimal_routes("XLM", "BTC", &edges, 100_000_000).unwrap();
    let result2 = optimizer.find_optimal_routes("XLM", "BTC", &edges, 100_000_000).unwrap();
    let result3 = optimizer.find_optimal_routes("XLM", "BTC", &edges, 100_000_000).unwrap();
...
fn test_benchmark_all_policies() {
    ...
    let results = optimizer.benchmark_policies("XLM", "BTC", &edges, 100_000_000).unwrap();
...
fn test_multi_hop_vs_direct() {
    ...
    let direct_result = optimizer.find_optimal_routes("XLM", "BTC", &edges, 50_000_000).unwrap();
    
    // Test multi-hop XLM -> USDC -> BTC route
    let multihop_result = optimizer.find_optimal_routes("XLM", "BTC", &edges, 50_000_000).unwrap();
